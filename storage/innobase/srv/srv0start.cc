  ...
  if (create_new_db) {
    ...
    for (i = 0; i < SRV_N_LOG_FILES_MAX; i++) {
      ...
      if (err == DB_NOT_FOUND) {
        ...
        /* 如果第i个文件不存在，说明前面已经打开了所有redolog文件 */
        /* opened all files */
        break;
      }
    ...
    /* n个redolog文件组成同一个space */
    /* fil_space_t可以用于tablespace，也可以重用于redolog文件组 */
    ...
    for (unsigned j = 0; j < i; j++) {
      ...
      /* 在log_space中注册具有file_pages个page的redolog文件 */
      if (fil_node_create(logfilename, static_cast<page_no_t>(file_pages),
                          log_space, false, false) == nullptr) {
        return (srv_init_abort(DB_ERROR));
      }
    ...
    /* 初始化log_sys结构体中的内容 */
    /* log_space_first_id表示一个伪的spaceid，因为redolog不需要space，只是借用space结构 */
  ...
  for (ulint count = 1;;) {
    ...
    if (wait) {
      ...
      switch (trx_purge_state()) {
        ...
        case PURGE_STATE_EXIT:
        case PURGE_STATE_DISABLED:
          srv_start_state &= ~S/RV_START_STATE_PURGE;
          break;
        case PURGE_STATE_RUN:
        case PURGE_STATE_STOP:
          ut_ad(0);
